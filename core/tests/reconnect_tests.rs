...
async fn configure_server_socket_for_test(
  // Renamed from configure_router_for_reconnect_test for generality
  socket: &Socket,
  endpoint_for_log: &str, // For logging context
  snd_timo_ms: i32,
  rcv_timo_ms: i32,
...
async fn test_push_pull_reconnect_advanced_style() -> Result<(), ZmqError> {
  ...
  let push_monitor_rx = configure_client_socket_for_reconnect_with_monitor(
    ...
    &endpoint,
    RECONNECT_IVL_MS_MR,
    CLIENT_SNDTIMEO_MS_MR,
    CLIENT_RCVTIMEO_MS_MR,
  )
  .await?;
  ...
  assert!(
    ...
    "[PUSH {}] Initial send should fail (ResourceLimitReached or Timeout)",
    endpoint
  );

  println!(
    "[SYS {}] Waiting {}ms before starting PULL server...",
    endpoint,
    SERVER_DOWNTIME_MR.as_millis()
  ...
  assert_eq!(
    ...
    test_payload,
    "[PULL {}] Received incorrect message",
    endpoint
  );
...
async fn test_req_rep_reconnect_advanced_style() -> Result<(), ZmqError> {
  ...
  assert!(
    ...
    "[REQ {}] Initial send should fail (ResourceLimitReached or Timeout)",
    endpoint
  );

  println!(
    "[SYS {}] Waiting {}ms before starting REP server...",
    endpoint,
    SERVER_DOWNTIME_MR.as_millis()
  );
  sleep(SERVER_DOWNTIME_MR).await;
  ...
  println!(
    "[REQ {}] Sending payload: '{}'",
    endpoint,
    String::from_utf8_lossy(req_payload)
  );
  ...
  println!(
    "[REP {}] Received: '{}'",
    endpoint,
    String::from_utf8_lossy(received_on_rep.data().unwrap())
  );
  assert_eq!(
    received_on_rep.data().unwrap(),
    req_payload,
    "[REP {}] Received incorrect request",
    endpoint
  );
  ...
  println!(
    "[REP {}] Sending reply: '{}'",
    endpoint,
    String::from_utf8_lossy(rep_payload)
  );
  ...
  println!(
    "[REQ {}] Received: '{}'",
    endpoint,
    String::from_utf8_lossy(received_on_req.data().unwrap())
  );
  assert_eq!(
    received_on_req.data().unwrap(),
    rep_payload,
    "[REQ {}] Received incorrect reply",
    endpoint
  );
...
async fn test_dealer_router_multiple_disconnect_reconnect_cycle() -> Result<(), ZmqError> {
  ...
  println!(
    "[DEALER {}] Creating DEALER socket (ID: {})...",
  ...
  for cycle in 1..=cycles {
    ...
    println!(
      "[ROUTER {}] Cycle {}: Creating and binding ROUTER socket...",
      endpoint, cycle
    );
    ...
    println!(
      "[DEALER {}] Cycle {}: Monitor confirmed HandshakeSucceeded: {:?}",
      endpoint, cycle, connect_event
    );
    ...
    println!(
      "[ROUTER {}] Cycle {}: Receiving message from DEALER...",
      endpoint, cycle
    );
    ...
    println!(
      "[ROUTER {}] Cycle {}: Received ID: {:?}",
      endpoint,
      cycle,
      String::from_utf8_lossy(router_recv_id.data().unwrap())
    );
    println!(
      "[ROUTER {}] Cycle {}: Received Payload: '{}'",
      endpoint,
      cycle,
    ...
    assert_eq!(
      ...
      "[ROUTER {}] Cycle {}: Incorrect dealer ID",
      endpoint,
      cycle
    );
    assert!(
      router_recv_id.is_more(),
      "[ROUTER {}] Cycle {}: Identity frame should have MORE flag as payload follows",
      endpoint,
      cycle
    );
    ...
    assert_eq!(
      ...
      msg_to_send.as_bytes(),
      "[ROUTER {}] Cycle {}: Incorrect payload",
      endpoint,
      cycle
    );
    assert!(
      !router_recv_payload.is_more(),
      "[ROUTER {}] Cycle {}: Payload frame should be the last part (no MORE flag)",
      endpoint,
      cycle
    );

    println!(
      "[ROUTER {}] Cycle {}: Received dealer message correctly.",
      endpoint, cycle
    );
    ...
    router_socket
      .send(Msg::from_vec(reply_to_send.as_bytes().to_vec()))
      .await?;
    ...
    println!(
      "[DEALER {}] Cycle {}: Monitor confirmed Disconnected: {:?}",
      endpoint, cycle, disconnect_event
    );

    if cycle < cycles {
      println!(
        "[SYS {}] Cycle {}: Server downtime for {}ms...",
        endpoint,
        cycle,
        SERVER_DOWNTIME_MR.as_millis()
      );
  ...
  println!(
    "--- test_dealer_router_multiple_disconnect_reconnect_cycle on {} PASSED ---",
    endpoint
  );
  ctx.term().await?;
  Ok(())
