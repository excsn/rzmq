...
mod tests {
  ...
  fn test_null_handshake_full_exchange() {
    ...
    assert_eq!(
      client_greeting.len(),
      GREETING_LENGTH,
      "client greeting must be 64 bytes"
    );
    assert_eq!(
      server_greeting.len(),
      GREETING_LENGTH,
      "server greeting must be 64 bytes"
    );
    ...
    assert!(
      !client_ready.is_empty(),
      "client must send READY after receiving server greeting"
    );
    assert_eq!(client.phase, ZmtpPhase::Ready);
    assert!(
      !has_handshake_complete(&client_out),
      "client HandshakeComplete must not fire yet"
    );

    // Server transitions to Ready but does not send anything yet.
    assert!(
      collect_sends(&server_out).is_empty(),
      "server must NOT send anything after client greeting"
    );
    assert_eq!(server.phase, ZmtpPhase::Ready);
    assert!(
      !has_handshake_complete(&server_out),
      "server HandshakeComplete must not fire yet"
    );
    ...
    assert!(
      !server_ready.is_empty(),
      "server must send READY after receiving client READY"
    );
    assert!(
      has_handshake_complete(&server_out2),
      "server must emit HandshakeComplete"
    );
    ...
    assert!(
      collect_sends(&client_out2).is_empty(),
      "client must not send anything after server READY"
    );
    assert!(
      has_handshake_complete(&client_out2),
      "client must emit HandshakeComplete"
    );
  ...
  fn test_null_handshake_flow_server_side() {
    ...
    assert_eq!(
      start_out.net_actions.len(),
      1,
      "expected server greeting from start()"
    );
    ...
    assert!(
      out.net_actions.is_empty(),
      "no sends expected after client greeting (server waits for client READY)"
    );
