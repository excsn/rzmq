...
impl fmt::Debug for UringOpRequest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            UringOpRequest::Nop { user_data, .. } => f.debug_struct("Nop").field("user_data", user_data).finish_non_exhaustive(),
            UringOpRequest::InitializeBufferRing { user_data, bgid, num_buffers, buffer_capacity, .. } => f
                .debug_struct("InitializeBufferRing")
                .field("user_data", user_data).field("bgid", bgid)
                .field("num_buffers", num_buffers).field("buffer_capacity", buffer_capacity)
                .finish_non_exhaustive(),
            UringOpRequest::RegisterRawBuffers { user_data, buffers, .. } => f
                .debug_struct("RegisterRawBuffers")
                .field("user_data", user_data).field("buffers_count", &buffers.len())
                .finish_non_exhaustive(),
            UringOpRequest::Listen { user_data, addr, protocol_handler_factory_id, .. } => f
                .debug_struct("Listen")
                .field("user_data", user_data).field("addr", addr)
                .field("protocol_handler_factory_id", protocol_handler_factory_id)
                .finish_non_exhaustive(),
            UringOpRequest::Connect { user_data, target_addr, protocol_handler_factory_id, .. } => {
                f.debug_struct("Connect")
                    .field("user_data", user_data).field("target_addr", target_addr)
                    .field("protocol_handler_factory_id", protocol_handler_factory_id)
                    .finish_non_exhaustive()
            }
            ...
            UringOpRequest::StartFdReadLoop { user_data, fd, .. } => f
                ...
                .finish_non_exhaustive(),
            UringOpRequest::SendDataViaHandler { user_data, fd, app_data, .. } => f
...
pub enum UringOpCompletion {
  ...
  OpError { user_data: UserData, op_name: String, error: ZmqError },
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            UringOpCompletion::InitializeBufferRingSuccess { user_data, bgid } => f
                ...
                .field("user_data", user_data).field("bgid", bgid).finish(),
            UringOpCompletion::RegisterRawBuffersSuccess { user_data } => f.debug_struct("RegisterRawBuffersSuccess").field("user_data", user_data).finish(),
            UringOpCompletion::ListenSuccess { user_data, listener_fd, actual_addr } => f
                .debug_struct("ListenSuccess")
                .field("user_data", user_data).field("listener_fd", listener_fd).field("actual_addr", actual_addr).finish(),
            UringOpCompletion::ConnectSuccess { user_data, connected_fd, peer_addr, local_addr } => f
            ...
            UringOpCompletion::ShutdownConnectionHandlerComplete { user_data, fd } => f
                ...
                .field("user_data", user_data).field("fd", fd).finish(),
            UringOpCompletion::OpError { user_data, op_name, error } => f
                .debug_struct("OpError")
                .field("user_data", user_data).field("op_name", op_name).field("error", error).finish(),
        }
    }
}