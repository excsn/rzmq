...
pub(crate) enum WorkItem {
  /// An external operation request from an API caller (e.g., connect, shutdown).
  ExternalOp(UringOpRequest),

  /// A batch of I/O blueprints generated by a connection handler.
  /// The `RawFd` identifies the connection this work belongs to.
  BlueprintBatch(RawFd, Vec<HandlerSqeBlueprint>),

  /// Application data (a full logical message) from a socket that needs to be sent.
  /// The `RawFd` identifies the connection this work belongs to.
  ApplicationData(RawFd, Arc<Vec<Msg>>),

  /// A request to initiate the close sequence for a specific connection.
  InitiateClose(RawFd),
}
