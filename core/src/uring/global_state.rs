...
static PARSED_MSG_TX_FOR_WORKER_TO_PROCESSOR: OnceCell<Mutex<Option<KanalSyncSender<(RawFd, HandlerUpstreamEvent)>>>> = OnceCell::new();
static PARSED_MSG_RX_FOR_PROCESSOR: OnceCell<Mutex<Option<KanalReceiver<(RawFd, HandlerUpstreamEvent)>>>> = OnceCell::new();
...
pub(crate) fn unregister_uring_fd_socket_core_mailbox(fd: RawFd) {
  if let Some(map_arc) = URING_FD_TO_SOCKET_CORE_MAILBOX_MAP.get() {
    ...
    if map_arc.write().remove(&fd).is_some() {
      ...
      warn!(raw_fd = fd, "GlobalUringState: URING_FD_TO_SOCKET_CORE_MAILBOX_MAP OnceCell not initialized. Cannot unregister FD.");
  }
}
