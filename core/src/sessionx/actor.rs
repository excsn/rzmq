    ...
    async fn run_loop(mut self) {
        ...
        // in the mailbox until the handshake is fully done.
        {
            ...
            'handshake: loop {
                ...
                let read_result =
                    tokio::time::timeout(hs_timeout, hs_read_half.read_buf(&mut self.handshake_read_buf))
                        .await;
                ...
                match read_result {
                    ...
                    Ok(Err(e)) => {
                        self.set_fatal_error(ZmqError::from_io_endpoint(e, "handshake read"))
                            .await;
                        break 'handshake;
                    }
                    Ok(Ok(0)) => {
            ...
            && !matches!(self.current_phase, ConnectionPhaseX::Terminating)
        {
            ...
            while self.current_phase == ConnectionPhaseX::WaitingForPipes {
                match self.command_mailbox_receiver.recv().await {
                    ...
                    Err(_) => {
                        tracing::info!(sca_handle = self.handle, "SCA Command mailbox closed while waiting for pipes.");
        ...
        if self.current_phase == ConnectionPhaseX::Operational {
            ...
            'operational: while self.current_phase == ConnectionPhaseX::Operational {
                    ...
                    }
                {
                    ...
                    let wire_size =
                        |msgs: &FrameBatch| msgs.iter().map(|m| m.size() + 9).sum::<usize>();
                    let hwm_budget =
                        sndhwm.saturating_sub(egress_buffer.pending_messages()).max(1);
                        ...
                        && total_bytes < max_bytes
                    {
                        ...
                        let _ = self.core_pipe_manager.try_recv_batch_from_core(
                            &mut core_batch_scratch,
                            max_count - outgoing_batch.len(),
                        );
                ...
                if self.zmtp_engine.is_waiting_for_pong() {
                    if let Some(deadline_std) = self.zmtp_engine.get_pong_deadline() {
                        ...
                        pong_timeout_future =
                            tokio::time::sleep_until(deadline_tokio).right_future();
                    }
                }
                        ...
                        match maybe_msgs_from_core {
                            Ok(first_msgs) => {
                                ...
                                while outgoing_batch.len() < max_count && total_bytes < max_bytes {
                                    ...
                                    for msgs in core_batch_scratch.drain(..) {
                                        if outgoing_batch.len() < max_count
                                            && total_bytes < max_bytes
                                        {
                                            total_bytes += wire_size(&msgs);
                                            outgoing_batch.push(msgs);
            ...
            match action {
                NetAction::Send { data, .. } => {
                    let wh = match self.write_half.as_mut() {
                        Some(w) => w,
                        None => {
                            self.set_fatal_error(ZmqError::Internal(
                    ...
                    match tokio::time::timeout(hs_timeout, wh.write_all(&data)).await {
                        Ok(Ok(())) => {}
                        Ok(Err(e)) => {
                            self.set_fatal_error(ZmqError::from_io_endpoint(e, "hs write"))
                                .await;
                            return;
                        }
                        Err(_) => {
                    ...
                    let _ = tokio::time::timeout(
                        hs_timeout,
                        self.write_half.as_mut().unwrap().flush(),
                    )
                    .await;
                }
                NetAction::SetCork(enable) => {
                ...
                Err(e) => {
                    ...
                    if self.error_for_drop_guard.is_none() {
                        self.error_for_drop_guard =
                            Some(ZmqError::from_io_endpoint(e, "graceful shutdown"));
                    }
                }
            }
        }
