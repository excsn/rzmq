...
use crate::runtime::{mailbox, ActorDropGuard, ActorType, Command, MailboxReceiver, MailboxSender, SystemEvent};
use crate::socket::events::{MonitorSender, SocketEvent};
use crate::socket::options::{
  self, parse_blob_option, parse_bool_option, parse_heartbeat_option, parse_i32_option, parse_keepalive_mode_option,
  parse_secs_duration_option, parse_timeout_option, parse_u32_option, SocketOptions, HEARTBEAT_IVL, HEARTBEAT_TIMEOUT,
  LINGER, PLAIN_PASSWORD, PLAIN_SERVER, PLAIN_USERNAME, RCVHWM, RCVTIMEO, RECONNECT_IVL, RECONNECT_IVL_MAX,
...
impl CoreState {
  ...
  pub(crate) fn send_monitor_event(&self, event: SocketEvent) {
    if let Some(ref tx) = self.monitor_tx {
      if let Err(e) = tx.send_blocking(event) {
    ...
    for (endpoint_uri_str, endpoint_info_val) in core_state.endpoints.iter() {
      ...
      if endpoint_info_val.endpoint_type == EndpointType::Session && endpoint_uri_str.starts_with("inproc://") {
        if let Some(pipe_ids_tuple_val) = endpoint_info_val.pipe_ids {
          ...
          self
            .inproc_connections_to_close
            .push((pipe_ids_tuple_val.0, pipe_ids_tuple_val.1, endpoint_uri_str.clone()));
        } else {
        ...
        if endpoint_info_val.handle_id > 0 {
          ...
          self
            .pending_children
            .insert(endpoint_info_val.handle_id, endpoint_uri_str.clone());
        } else {
...
impl SocketCore {
  ...
  async fn run_command_loop(
    ...
    let socket_type_for_log = core_arc.core_state.try_read().map(|s| s.socket_type);
        ...
        tokio::select! {
            ...
            match event_result {
              ...
              Ok(SystemEvent::InprocBindingRequest {
                  ...
                  let is_my_binding_name = {
                      core_arc.core_state.read().bound_inproc_names.contains(&target_inproc_name)
                  };
              ...
              Ok(SystemEvent::InprocPipePeerClosed { target_inproc_name, closed_by_connector_pipe_read_id }) => {
                ...
                if is_my_binding_name {
                  ...
                  {
                    let binder_core_state_guard = core_arc.core_state.read();
          ...
          _ = linger_check_interval.tick(), if current_shutdown_phase == ShutdownPhase::Lingering => {
            ...
            if coordinator_guard.current_phase() == ShutdownPhase::Lingering {
              if coordinator_guard.linger_deadline.is_none() && core_arc.core_state.read().options.linger != Some(Duration::ZERO) {
                let linger_opt_val = { core_arc.core_state.read().options.linger };
                coordinator_guard.start_linger(linger_opt_val, core_handle);
              }
              let queues_empty_check = coordinator_guard.are_queues_empty(&*core_arc.core_state.read());
              if coordinator_guard.is_linger_expired(core_handle) || queues_empty_check {
                ...
                {
                  let mut core_state_guard = core_arc.core_state.write();
    ...
    #[cfg(feature = "inproc")]
    {
      let bound_names_list = { std::mem::take(&mut core_arc.core_state.write().bound_inproc_names) };
      if !bound_names_list.is_empty() {
        ...
        let unbind_futs = bound_names_list.into_iter().map(|name_val| {
          ...
          async move {
            ctx_inner_clone_val.unregister_inproc(&name_val);
          }
        });
    ...
    let endpoints_to_stop = {
      ...
      coordinator
        ...
        .filter_map(|child_handle_id| {
          let state = core_arc.core_state.read();
          // Find the endpoint info corresponding to the child handle
          state
            .endpoints
            .values()
            .find(|info| info.handle_id == *child_handle_id)
      ...
      stop_futures.push(async move {
        if let Err(e) = mailbox.send(Command::Stop).await {
          tracing::warn!(
            ...
            "Failed to send Stop to child actor: {:?}",
            e
          );
        } else {
          tracing::trace!(
            parent_handle = core_handle,
            child_handle = child_handle,
            "Sent Stop command to child actor."
          );
        }
      });
    }
  ...
  async fn disconnect_inproc_connections(core_arc: Arc<SocketCore>) {
    ...
    tracing::debug!(
      handle = core_handle,
      "Finished attempting to disconnect inproc connections."
    );
  }
  ...
  async fn initiate_shutdown_phase(core_arc: Arc<SocketCore>) -> bool {
    ...
    let initiated_flag = {
      let mut core_state_guard = core_arc.core_state.write();
      coordinator.initiate(&mut core_state_guard, core_handle)
    };

    if initiated_flag {
      if coordinator.current_phase() == ShutdownPhase::Lingering {
        let are_queues_empty;
        {
          let core_state_guard = core_arc.core_state.read();
          let linger_opt_val = core_state_guard.options.linger;
          coordinator.start_linger(linger_opt_val, core_handle);
          are_queues_empty = coordinator.are_queues_empty(&core_state_guard);
        }

        if coordinator.is_linger_expired(core_handle) || are_queues_empty {
          let pipes_to_clean_val;
          {
            let mut core_state_guard = core_arc.core_state.write();
            coordinator.advance_to_cleaning(core_handle, &mut core_state_guard);
            #[cfg(feature = "inproc")]
            {
              pipes_to_clean_val = coordinator.inproc_connections_to_close.clone();
            }

            #[cfg(not(feature = "inproc"))]
            {
              pipes_to_clean_val = Vec::new();
            }
  ...
  async fn perform_pipe_cleanup(
    ...
    if !matches!(
      coordinator_val.state,
      ShutdownPhase::CleaningPipes | ShutdownPhase::Finished
    ) {
    ...
    {
      {
        let mut state_val = core_arc.core_state.write();
      ...
      #[cfg(feature = "inproc")]
      {
        tracing::debug!(
          handle = core_handle,
          num_inproc_pipes = pending_inproc_pipes_to_cleanup.len(),
          "Removing EndpointInfo for cleaned inproc pipes."
        );
        let mut state_val = core_arc.core_state.write();
    ...
    for (id_val, sender_val) in pipes_tx_map.iter() {
      if !sender_val.is_closed() {
        // Only log if not already closed
        sender_val.close();
        tracing::trace!(handle = core_handle, pipe_id = id_val, "Closed pipe sender in cleanup.");
      }
    }
    if !pipes_tx_map.is_empty() {
      // Log if any were taken, even if already closed
      tracing::debug!(
        handle = core_handle,
        count = pipes_tx_map.len(),
        "Pipe sender map processed."
      );
    }
    ...
    if !reader_tasks_map.is_empty() {
      ...
      tracing::debug!(
        handle = core_handle,
        count = reader_handles_list.len(),
        "All pipe reader tasks aborted."
      );
    } else {
    ...
    #[cfg(feature = "inproc")]
    {
      ...
      for ev in peer_closed_events {
        ...
        if event_bus.publish(ev).is_err() {
          tracing::warn!(
            handle = core_handle,
            "Failed to publish InprocPipePeerClosed event during cleanup."
          );
        }
      }
    }
  ...
  async fn handle_user_monitor(
    ...
    let mut state_guard = core_arc.core_state.write();
  ...
  async fn handle_user_bind(
    ...
    match parse_result {
        ...
        if state_g.endpoints.contains_key(uri) {
          ...
          match listener_res {
            Ok((child_mb, child_th, resolved_tcp_uri)) => {
              let mut state_w = core_arc.core_state.write();
      ...
      Ok(Endpoint::Inproc(ref name)) => {
        ...
        let is_bound = core_c.core_state.read().bound_inproc_names.contains(&name_c);
        if is_bound {
    ...
    if final_user_bind_result.is_ok() {
      if let Some(ref actual_uri) = actual_uri_for_state_update {
        ...
        state_w.send_monitor_event(SocketEvent::Listening {
  ...
  async fn handle_user_disconnect(
    ...
    if let Some(info) = core_arc.core_state.read().endpoints.get(&endpoint) {
      if info.endpoint_type == EndpointType::Session {
        child_id_to_signal = Some(info.handle_id);
        resolved_uri_to_stop = Some(endpoint.clone());
      }
    } else {
      for (res_uri, info) in core_arc.core_state.read().endpoints.iter() {
    ...
    if let Some(child_id_val) = child_id_to_signal {
      if let Some(res_uri_val) = resolved_uri_to_stop {
        ...
        if core_arc.core_state.write().endpoints.remove(&res_uri_val).is_some() {
  ...
  async fn handle_user_unbind(
    ...
    if let Some((uri, mailbox, _child_id)) = listener_info_opt {
      if core_arc.core_state.write().endpoints.remove(&uri).is_some() {
      ...
      #[cfg(feature = "inproc")]
      {
        ...
        if !name.is_empty() {
          let did_remove_inproc_name = {
            let mut cs_recheck = core_arc.core_state.write();
            cs_recheck.bound_inproc_names.remove(name)
          };

          if did_remove_inproc_name {
  ...
  async fn handle_conn_success(
    ...
    let is_this_an_outbound_connection = {
      ...
      for (ep_uri_key, ep_info_val) in core_arc.core_state.read().endpoints.iter() {
    ...
    core_arc.core_state.write().endpoints.insert(
      endpoint.clone(),
      EndpointInfo {
    ...
    if session_mailbox.is_closed() {
      ...
      let mut sg_cleanup = core_arc.core_state.write();
  ...
  async fn handle_conn_failed(core_arc: Arc<SocketCore>, target_uri: String, error: ZmqError) {
    ...
    core_arc.core_state.read().send_monitor_event(event_payload);
  ...
  async fn process_inproc_binding_request(
    ...
    if accept_result.is_ok() {
      ...
      let monitor_tx_for_binder_event = core_arc.core_state.read().get_monitor_sender_clone();
  ...
  async fn process_child_completion(
    ...
    if matches!(
      ...
      ShutdownPhase::StoppingChildren
        | ShutdownPhase::Lingering
        | ShutdownPhase::CleaningPipes
        | ShutdownPhase::Finished // Include finished
    ) {
      if is_err {
    ...
    if matches!(
      current_phase_val,
      ShutdownPhase::StoppingChildren | ShutdownPhase::Lingering
    ) {
    ...
    if state_transitioned_to_linger {
      ...
      let linger_o = {
        let core_state_guard = core_arc.core_state.read();
        core_state_guard.options.linger
      };

      coordinator_g.start_linger(linger_o, core_h); // Start linger timer/state

      if coordinator_g.is_linger_expired(core_h) || coordinator_g.are_queues_empty(&core_arc.core_state.read()) {
        tracing::debug!(
          handle = core_h,
          "process_child_completion: Last child stopped and linger condition met, advancing to cleaning."
        );
        // Pass the mutable core_state_guard here
        coordinator_g.advance_to_cleaning(core_h, &mut core_arc.core_state.write());
    ...
    if let Some(uri_s) = endpoint_uri_opt {
      ...
      let needs_clean = {
        let state_guard = core_arc.core_state.read();
      ...
      if needs_clean {
        ...
        if current_phase_val == ShutdownPhase::Running && is_err {
          // Check if stopped unexpectedly while core is running
    ...
    let current_core_phase_now = {
      // Re-check current phase after cleanup and potential shutdown progression
      let guard = core_arc.shutdown_coordinator.lock().await;
      guard.current_phase()
    }; // Guard dropped

    if current_core_phase_now == ShutdownPhase::Running {
      if let Some(ref removed_info) = removed_endpoint_info_for_reconnect {
        // Child died unexpectedly while core was running
        if removed_info.endpoint_type == EndpointType::Session && removed_info.is_outbound_connection {
          // Check the new flag
          if let Some(ref target_uri_to_reconnect) = removed_info.target_endpoint_uri {
            ...
            if !target_uri_to_reconnect.starts_with("inproc://") {
              let reconnect_enabled = {
                let cs_guard = core_arc.core_state.read();
      ...
      if removed_endpoint_info_for_reconnect.is_some() {
        // Log if we *would* have reconnected
        tracing::debug!(
          handle = core_h,
          child_handle = child_handle,
  ...
  async fn cleanup_session_state_by_uri(
    ...
    let monitor_tx_c = core_arc.core_state.read().get_monitor_sender_clone();
    let mut ep_type_removed_opt: Option<EndpointType> = None;
    let mut actual_removed_ep_info: Option<EndpointInfo> = None;

    {
      let core_state_guard = core_arc.core_state.upgradable_read();
      if let Some(info_at_uri_val) = core_state_guard.endpoints.get(endpoint_uri) {
        if info_at_uri_val.handle_id == stopping_child_handle_id {
          let mut core_state_guard = RwLockUpgradableReadGuard::upgrade(core_state_guard);

          if let Some(removed_val) = core_state_guard.endpoints.remove(endpoint_uri) {
            ...
            if let Some((write_id, read_id)) = actual_removed_ep_info.as_ref().unwrap().pipe_ids {
              ...
              if core_state_guard.remove_pipe_state(write_id, read_id) {
  ...
  async fn cleanup_session_state_by_pipe(
    ...
    for (uri_val, info_val) in core_arc.core_state.read().endpoints.iter() {
  ...
  async fn cleanup_session_state_by_pipe_ids(
    ...
    let pipes_removed_f = core_arc
      .core_state
      .write()
      .remove_pipe_state(pipe_write_id, pipe_read_id);
    let mut uri_of_ep_to_remove_opt: Option<String> = None;
    for (uri_v, info_v) in core_arc.core_state.read().endpoints.iter() {
    ...
    if let Some(uri_v_val) = uri_of_ep_to_remove_opt {
      if let Some(removed_info_v) = core_arc.core_state.write().endpoints.remove(&uri_v_val) {
    ...
    #[cfg(feature = "io-uring")]
    {
      ...
      if option == IO_URING_SNDZEROCOPY {
        ...
        tracing::debug!(
          ...
          state_g.options.io_uring_send_zerocopy
        );
        return Ok(());
        ...
        tracing::debug!(
          ...
          state_g.options.io_uring_recv_multishot
        );
        return Ok(());
      }
