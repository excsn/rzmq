...
use crate::runtime::{MailboxSender, Command, SystemEvent}; // Command, SystemEvent not directly used in structs but for context
...
impl CoreState {
    ...
    pub(crate) fn remove_pipe_state(&mut self, pipe_write_id: usize, pipe_read_id: usize) -> bool {
        ...
        if tx_removed {
            tracing::trace!(core_handle = self.handle, pipe_id = pipe_write_id, "CoreState: Removed pipe sender");
        }

        let reader_removed = if let Some(handle) = self.pipe_reader_task_handles.remove(&pipe_read_id) {
            tracing::trace!(core_handle = self.handle, pipe_id = pipe_read_id, "CoreState: Aborting pipe reader task");
            handle.abort();
            true
        } else {
            false
        ...
        if reader_removed {
            tracing::trace!(core_handle = self.handle, pipe_id = pipe_read_id, "CoreState: Removed pipe reader task handle");
        }

        let map_removed = self.pipe_read_id_to_endpoint_uri.remove(&pipe_read_id).is_some();
        if map_removed {
             tracing::trace!(core_handle = self.handle, pipe_id = pipe_read_id, "CoreState: Removed pipe_read_id_to_endpoint_uri mapping");
        }
    ...
    pub(crate) fn send_monitor_event(&self, event: SocketEvent) {
        if let Some(ref tx) = self.monitor_tx {
            if tx.try_send(event).is_err() { // Non-blocking send
                tracing::warn!(socket_handle = self.handle, "Failed to send event to monitor channel (full or closed)");
            }
        }
    }
