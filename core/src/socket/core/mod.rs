...
pub struct SocketCore {
    ...
    pub(crate) command_sender: MailboxSender, // Sender for its own command mailbox
    pub(crate) core_state: parking_lot::RwLock<CoreState>, // State is frequently accessed, parking_lot::RwLock is good.
...
impl SocketCore {
    pub(crate) fn create_and_spawn(
        ...
        { // Scope for TokioRwLock write guard
            let mut socket_logic_weak_ref_guard = socket_core_arc.socket_logic.try_write()
                .map_err(|_| ZmqError::Internal("SocketCore: TokioRwLock fair lock poisoned or unavailable during ISocket weak ref init (try_write)".into()))?;
        ...
        let _core_task_join_handle = tokio::spawn(
    ...
    pub(crate) async fn get_socket_logic(&self) -> Option<Arc<dyn ISocket>> {
        self.socket_logic.read().await.as_ref().and_then(|weak_ref| weak_ref.upgrade())
    }
