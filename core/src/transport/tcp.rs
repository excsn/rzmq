...
impl TcpListener {
  ...
  pub(crate) fn create_and_spawn(
    ...
    let addr_for_socket2_parse = if bind_addr_str_for_parse == "0.0.0.0:0" || bind_addr_str_for_parse.starts_with("[::]:0") {
        if bind_addr_str_for_parse.starts_with("[::]") { "::1:0" } else { "127.0.0.1:0" }
    } else { bind_addr_str_for_parse };
    let parsed_socket_addr: StdSocketAddr = addr_for_socket2_parse.parse().map_err(|e| 
        ZmqError::InvalidEndpoint(format!("Parse bind address '{}': {}", addr_for_socket2_parse, e)))?;

    let domain = if parsed_socket_addr.is_ipv4() { socket2::Domain::IPV4 } else { socket2::Domain::IPV6 };
    ...
    let addr_for_bind_call = bind_addr_str_for_parse.parse::<StdSocketAddr>().map_err(|e| 
        ZmqError::InvalidEndpoint(format!("Parse bind address for bind call'{}': {}", bind_addr_str_for_parse, e)))?;
    s.bind(&addr_for_bind_call.into()).map_err(|e| ZmqError::from_io_endpoint(e, &endpoint))?;
    s.listen(options.backlog.unwrap_or(128) as i32).map_err(ZmqError::from)?;
    ...
    let accept_loop_task_jh = tokio::spawn(TcpListener::run_accept_loop(
        ...
        Arc::new(tokio_listener), transport_cfg.clone(), options.clone(), 
  ...
  async fn run_accept_loop(
    ...
    loop {
      let permit = match connection_limiter.clone().acquire_owned().await {
        Ok(p) => p,
        Err(_) => { loop_error_to_report = Some(ZmqError::Internal("Connection limiter closed".into())); break; }
      };
      ...
      match listener.accept().await {
        Ok((tokio_tcp_stream, peer_addr)) => {
          ...
          tracing::info!("Accepted TCP connection from {} (for listener {})", peer_addr_str, endpoint_uri);
          ...
          tokio::spawn({
            ...
            async move {
                      ...
                      if let Err(e) = apply_tcp_socket_options_to_std(&std_stream, &transport_config_clone) {
                        ...
                        if let Err(e) = worker_op_tx.as_async().send(register_fd_req).await {
                          ...
                          match reply_rx_for_op.await {
                            ...
                            Ok(Err(worker_err)) => { 
                              tracing::error!("Register accepted FD {} with UringWorker failed (worker error): {:?}", raw_fd, worker_err); 
                              unsafe { let _ = libc::close(raw_fd); } 
                              setup_successful = false; 
                            }
                            Err(oneshot_recv_err) => { 
                              tracing::error!("Register accepted FD {} with UringWorker failed (reply channel error): {:?}", raw_fd, oneshot_recv_err); 
                              unsafe { let _ = libc::close(raw_fd); } 
                              setup_successful = false; 
                            }
                          }
              ...
              } else { // Standard path: SessionBase + ZmtpEngineCoreStd
                if let Err(e) = apply_tcp_socket_options_to_tokio(&tokio_tcp_stream, &transport_config_clone) {
                  tracing::error!("Opt apply failed (tokio stream) for {}: {}. Dropping.", peer_addr_str, e); 
                  setup_successful = false;
                } else {
...
impl TcpConnecter {
  ...
  pub(crate) fn create_and_spawn( 
    ...
    context_handle_source: Arc<std::sync::atomic::AtomicUsize>,
    monitor_tx: Option<MonitorSender>, context: Context, parent_socket_id: usize,
  ) -> JoinHandle<()> {
    let actor_type = ActorType::Connecter;
    let transport_config = TcpTransportConfig {
      tcp_nodelay: options.tcp_nodelay, keepalive_time: options.tcp_keepalive_idle,
      keepalive_interval: options.tcp_keepalive_interval, keepalive_count: options.tcp_keepalive_count,
    };
    let connecter_actor = TcpConnecter {
      handle, endpoint: endpoint.clone(), config: transport_config,
  ...
  async fn run_connect_loop(mut self, monitor_tx: Option<MonitorSender>) { 
    ...
    let target_socket_addr: StdSocketAddr = match endpoint_uri_clone.strip_prefix("tcp://") {
      Some(addr_str) => match addr_str.parse() {
        ...
        Err(e) => { 
            ...
            self.context.publish_actor_stopping(connecter_handle, actor_type, Some(endpoint_uri_clone), Some(err)); 
            return;
        }
      },
      None => { 
    ...
    let initial_reconnect_ivl = self.context_options.reconnect_ivl.unwrap_or(Duration::from_millis(DEFAULT_RECONNECT_IVL_MS));
    let no_retry_after_first_connect_fail = self.context_options.reconnect_ivl == Some(Duration::ZERO) && 
                                            (self.context_options.reconnect_ivl_max == Some(Duration::ZERO) || self.context_options.reconnect_ivl_max.is_none());
    let mut current_retry_delay = if self.context_options.reconnect_ivl == Some(Duration::ZERO) && self.context_options.reconnect_ivl_max.map_or(false, |d| d > Duration::ZERO) {
      self.context_options.reconnect_ivl_max.unwrap()
    } else { initial_reconnect_ivl };
    ...
    'connecter_life_loop: loop {
      if attempt_count > 0 { 
        ...
        match self.wait_for_retry_delay_internal(&mut system_event_rx, current_retry_delay, &monitor_tx, attempt_count + 1).await {
          Ok(true) => {}
          Ok(false) => { last_connect_attempt_error = Some(ZmqError::Internal("Connecter shutdown by event retry delay.".into())); break 'connecter_life_loop; }
          Err(()) => { last_connect_attempt_error = Some(ZmqError::Internal("Connecter shutdown error retry delay.".into())); break 'connecter_life_loop; }
        }
        if current_retry_delay > Duration::ZERO && reconnect_ivl_max_opt.map_or(true, |max_d| max_d > Duration::ZERO) {
          if let Some(max_d) = reconnect_ivl_max_opt { current_retry_delay = (current_retry_delay * 2).min(max_d); } 
          else { current_retry_delay = initial_reconnect_ivl; }
      ...
      match system_event_rx.try_recv() { 
        Ok(SystemEvent::ContextTerminating) => { last_connect_attempt_error = Some(ZmqError::Internal("Shutdown by ContextTerminating (pre-connect).".into())); break 'connecter_life_loop; }
        Ok(SystemEvent::SocketClosing { socket_id: sid }) if sid == self.parent_socket_id => { last_connect_attempt_error = Some(ZmqError::Internal("Shutdown by parent SocketClosing (pre-connect).".into())); break 'connecter_life_loop; }
        Err(broadcast::error::TryRecvError::Closed) => { last_connect_attempt_error = Some(ZmqError::Internal("Event bus closed (pre-connect).".into())); break 'connecter_life_loop; }
        Err(broadcast::error::TryRecvError::Lagged(_)) => { last_connect_attempt_error = Some(ZmqError::Internal("Event bus lagged (pre-connect).".into())); break 'connecter_life_loop; }
        _ => {} 
      }
      ...
      match single_attempt_outcome {
        ...
        Err(attempt_failure_error) => { 
          ...
          if is_fatal_connect_error(&attempt_failure_error) || matches!(&attempt_failure_error, ZmqError::Internal(s) if s.contains("shutdown by") || s.contains("event bus error")) {
          ...
          if attempt_count == 1 { 
            if let Some(ref mon_tx) = monitor_tx {
              let _ = mon_tx.try_send(SocketEvent::ConnectDelayed { endpoint: endpoint_uri_clone.clone(), error_msg: attempt_failure_error.to_string(), });
            }
          }
        }
      }
    ...
    if let Some(ref final_err) = last_connect_attempt_error { 
      if !matches!(final_err, ZmqError::Internal(s) if s.contains("shutdown by") || s.contains("event bus error")) {
        let _ = self.context.event_bus().publish(SystemEvent::ConnectionAttemptFailed {
          parent_core_id: self.parent_socket_id, target_endpoint_uri: endpoint_uri_clone.clone(), error_msg: final_err.to_string(),
        });
        if let Some(ref mon_tx_final) = monitor_tx {
          let _ = mon_tx_final.try_send(SocketEvent::ConnectFailed { endpoint: endpoint_uri_clone.clone(), error_msg: final_err.to_string(), });
        }
      }
    }
    
    self.context.publish_actor_stopping(connecter_handle, actor_type, Some(endpoint_uri_clone), last_connect_attempt_error);
        ...
        #[cfg(feature = "io-uring")]
        {
          ...
          match reply_rx_for_op.await {
            ...
            Ok(Ok(other_completion)) => { 
              tracing::error!("UringWorker bad success for RegisterExternalFd (fd {}): {:?}", raw_fd, other_completion);
              unsafe { let _ = libc::close(raw_fd); } 
              Err(ZmqError::Internal(format!("UringWorker unexpected success: {:?}", other_completion))) 
            }
            Ok(Err(worker_err)) => { 
              tracing::error!("Register FD {} with UringWorker failed (worker error): {:?}", raw_fd, worker_err); 
              unsafe { let _ = libc::close(raw_fd); } 
              Err(worker_err) 
            }
            Err(oneshot_recv_err) => { 
              tracing::error!("Register FD {} with UringWorker failed (reply channel error): {:?}", raw_fd, oneshot_recv_err); 
              unsafe { let _ = libc::close(raw_fd); } 
              Err(ZmqError::Internal(format!("Register FD with UringWorker reply error: {:?}", oneshot_recv_err))) 
            }
          }
        }
...
pub(crate) fn is_fatal_connect_error(e: &ZmqError) -> bool {
  match e {
    ZmqError::IoError { kind, .. } => {
      matches!(kind, io::ErrorKind::AddrNotAvailable | io::ErrorKind::AddrInUse | io::ErrorKind::InvalidInput | io::ErrorKind::PermissionDenied | io::ErrorKind::ConnectionRefused )
    }
    ZmqError::InvalidEndpoint(_) | ZmqError::UnsupportedTransport(_) | ZmqError::SecurityError(_) | ZmqError::AuthenticationFailure(_) => true,
    _ => false,
  }
}
...
fn apply_socket2_options_pre_connect(socket: &socket2::Socket, config: &TcpTransportConfig) -> Result<(), ZmqError> {
  ...
  if config.keepalive_time.is_some() || config.keepalive_interval.is_some() || config.keepalive_count.is_some() {
    ...
    if let Some(time) = config.keepalive_time { keepalive = keepalive.with_time(time); }
    #[cfg(any(unix, target_os = "windows"))]
    if let Some(interval) = config.keepalive_interval { keepalive = keepalive.with_interval(interval); }
    #[cfg(unix)]
    if let Some(count) = config.keepalive_count { keepalive = keepalive.with_retries(count); }
    socket.set_tcp_keepalive(&keepalive).map_err(ZmqError::from)?;
  }
  Ok(())
}

fn apply_tcp_socket_options_to_tokio(stream: &tokio::net::TcpStream, config: &TcpTransportConfig) -> Result<(), ZmqError> {
