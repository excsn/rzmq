...
async fn main() -> Result<(), ZmqError> {
  ...
  dealer_socket.set_option(zmq_opts::IO_URING_SESSION_ENABLED, true).await?;
      ...
      Ok(reply_msg) => {
        ...
        if reply_msg.size() == payload_size_bytes {
          ...
          eprintln!(
            ...
            payload_size_bytes, reply_msg.size(), i
          );
        }
      }
      Err(ZmqError::InvalidState(ref msg)) if msg.contains("Socket is closing") => {
  ...
  if seconds > 0.0 {
    ...
    println!("DEALER: Send rate: {:.2} msgs/sec, {:.2} MB/sec", send_msg_rate, send_byte_rate_mb);

    let recv_msg_rate = messages_received as f64 / seconds;
    let recv_byte_rate_mb = (bytes_received as f64 / seconds) / (1024.0 * 1024.0);
    println!("DEALER: Recv rate: {:.2} msgs/sec, {:.2} MB/sec", recv_msg_rate, recv_byte_rate_mb);
  }
  if messages_sent != messages_received {
      eprintln!("WARNING: Message sent count ({}) does not match received count ({}).", messages_sent, messages_received);
  }
  ...
  match router_task_handle.await {
    Ok((router_msgs, router_bytes_val)) => {
      println!("ROUTER: Echo task completed. Messages echoed: {}. Bytes echoed: {}", router_msgs, router_bytes_val);
      if seconds > 0.0 {
