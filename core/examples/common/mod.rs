...
) -> Vec<Result<String, String>> { // Returns Ok(description) on success, Err(description + error) on failure
        ...
        handshake_futures.push(tokio::spawn(async move {
            ...
            loop {
                ...
                match tokio::time::timeout(timeout_duration.saturating_sub(wait_start.elapsed()), monitor_rx.recv()).await {
                    Ok(Ok(event)) => {
                        if print_handshake_events { // Assuming PRINT_HANDSHAKE_EVENTS is a const bool available in scope
                            println!("[{}] Monitor received event: {:?}", description, event);
                        }
                        if let SocketEvent::HandshakeSucceeded { endpoint: event_endpoint } = event {
                            if event_endpoint == expected_endpoint_clone {
                                println!(
                                    "[{}] HandshakeSucceeded for {} after {:?}.",
                                    desc_clone_success, event_endpoint, wait_start.elapsed()
                                );
                    ...
                    Ok(Err(recv_err)) => { // Monitor channel closed or other recv error
                        let err_msg = format!(
                    ...
                    Err(_select_timeout_error) => { // Outer timeout (should be caught by wait_start.elapsed())
