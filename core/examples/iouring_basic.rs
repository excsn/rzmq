...
use rzmq::socket::options as rzmq_options; // Alias for clarity
...
async fn main() -> Result<(), ZmqError> {
    ...
    let rep_handle = tokio::spawn(async move {
        ...
        loop {
            match rep.recv().await { // Assuming REP expects single part messages
                Ok(req_msg) => {
                    ...
                    if let Err(e) = rep.send(req_msg).await { // Echo back the received Msg
    ...
    for i in 0..3 {
        ...
        match req.recv().await {
            Ok(reply) => {
                println!("REQ: Received reply '{}'", String::from_utf8_lossy(reply.data().unwrap_or_default()));
            }
            Err(e) => {
