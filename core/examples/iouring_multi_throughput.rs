...
fn extract_reply_data(mut frames: Vec<Msg>) -> Option<(RequestId, Vec<Msg>)> {
  ...
  if frames[0].size() == 0 {
    // ZMTP empty delimiter prepended by ROUTER socket logic
    if PRINT_INDIVIDUAL_MESSAGES {
      println!("[DEALER Central Receiver] extract_reply_data: Consuming leading empty delimiter frame from ROUTER.");
    }
    frames.remove(0);
    if frames.is_empty() {
      if PRINT_INDIVIDUAL_MESSAGES {
        eprintln!("[DEALER Central Receiver] extract_reply_data: No frames after empty delimiter.");
      }
      return None;
    }
  }
  if frames.is_empty() {
    if PRINT_INDIVIDUAL_MESSAGES {
      eprintln!("[DEALER Central Receiver] extract_reply_data: No request_id frame found after delimiter processing.");
    }
    return None;
  }
  ...
  let request_id_bytes = match request_id_frame.data() {
    ...
    None => {
      if PRINT_INDIVIDUAL_MESSAGES {
        eprintln!("[DEALER Central Receiver] extract_reply_data: request_id_frame has no data.");
      }
      return None;
    }
  };
...
) -> Result<(u64, u64, u64), ZmqError> {
  // Returns (echoed_msgs, echoed_bytes, timeouts)
  ...
  while received_count < total_expected_messages {
    match router_socket.recv_multipart().await {
      Ok(mut received_parts) => {
        ...
        if PRINT_INDIVIDUAL_MESSAGES {
          let request_id_from_payload =
            if !received_parts.is_empty() && received_parts[0].data().map_or(0, |d| d.len()) == 8 {
          ...
          println!(
            ...
            received_parts.len().saturating_sub(1)
          );
        }
        ...
        if let Err(e) = router_socket.send_multipart(reply_to_send).await {
          eprintln!(
            "[ROUTER] Error sending reply for received message #{}: {}",
            received_count, e
          );
          return Err(e);
        }
      ...
      Err(ZmqError::Timeout) => {
        ...
        if PRINT_INDIVIDUAL_MESSAGES || timeouts_occurred % 100 == 0 {
          eprintln!(
            "[ROUTER] Recv timeout (total timeouts: {}). Still expecting {} of {} total messages.",
            timeouts_occurred,
            total_expected_messages.saturating_sub(received_count),
            total_expected_messages
          );
        }
        if timeouts_occurred > (total_expected_messages / 100).max(20) && received_count < total_expected_messages {
          eprintln!(
            "[ROUTER] Excessive timeouts ({}). Terminating echo task prematurely. Received {}/{} messages.",
      ...
      Err(ZmqError::InvalidState(ref msg)) if msg.contains("Socket is closing") => {
        println!(
          "[ROUTER] Socket is closing, terminating echo task. Received {} messages.",
          received_count
        );
        break;
      }
      Err(e) => {
        eprintln!(
          "[ROUTER] recv_multipart error after receiving {} messages: {}. Terminating echo task.",
          received_count, e
        );
        return Err(e);
      }
...
async fn run_dealer_central_receiver(
  ...
  // (replies_processed, bytes_replied, extraction_errors, last_reply_timestamp)
  println!(
  ...
  while replies_processed + extraction_errors < total_expected_replies {
    match dealer_socket.recv_multipart().await {
      Ok(mut frames_from_dealer_socket) => {
        ...
        if PRINT_INDIVIDUAL_MESSAGES {
          println!(
            "[DEALER Central Receiver] Raw frames from socket (count: {}): sizes {:?}",
        ...
        if let Some((request_id, reply_payload_parts)) = extract_reply_data(frames_from_dealer_socket) {
          if PRINT_INDIVIDUAL_MESSAGES {
            println!(
              "[DEALER Central Receiver] Extracted RequestId: {}, with {} payload parts.",
              request_id,
              reply_payload_parts.len()
            );
          }
          ...
          if let Some(_permit_to_drop) = map_guard.remove(&request_id) {
            ...
            if PRINT_INDIVIDUAL_MESSAGES {
              println!(
                "[DEALER Central Receiver] Processed reply for RequestId {}, permit released.",
                request_id
              );
            }
          } else {
      ...
      Err(ZmqError::InvalidState(ref msg)) if msg.contains("Socket is closing") => {
        println!(
          "[DEALER Central Receiver] Main DEALER socket closing (likely signaled by main). Terminating receiver task."
        );
        break;
      }
      Err(ZmqError::Timeout) => {
        if PRINT_INDIVIDUAL_MESSAGES {
          println!("[DEALER Central Receiver] Main DEALER socket recv timed out.");
        }
        if last_message_activity_time.elapsed() > Duration::from_millis(CENTRAL_RECEIVER_IDLE_TIMEOUT_MS)
          && (replies_processed + extraction_errors < total_expected_replies)
        {
          // Only break if idle AND still expecting more
          println!(
            "[DEALER Central Receiver] Idle for over {}ms. Processed {} of {} expected replies. Terminating.",
            CENTRAL_RECEIVER_IDLE_TIMEOUT_MS,
            replies_processed + extraction_errors,
            total_expected_replies
          );
          break;
        }
        if replies_processed + extraction_errors >= total_expected_replies {
      ...
      Err(e) => {
        eprintln!(
          "[DEALER Central Receiver] Error receiving from main DEALER socket: {}. Terminating.",
          e
        );
        break;
      }
...
async fn run_dealer_task(
  ...
  for j in 0..num_messages_for_this_task {
    let permit = match shared_semaphore.clone().acquire_owned().await {
      ...
      Err(_) => {
        eprintln!(
          "[DEALER {}] Shared semaphore closed while acquiring for send (idx {}). Aborting task.",
          dealer_id, j
        );
    ...
    match dealer_socket_shared_clone.send_multipart(app_frames_to_send).await {
      ...
      Err(e) => {
        ...
        if let Some(permit_to_drop) = pending_requests_map_clone.lock().await.remove(&request_id) {
          ...
          if PRINT_INDIVIDUAL_MESSAGES {
            println!(
              "[DEALER {}] Send error path: Permit for RequestId {} released.",
          ...
          if PRINT_INDIVIDUAL_MESSAGES {
            println!(
              "[DEALER {}] Send error path: No permit found in map for failed RequestId {}.",
              dealer_id, request_id
            );
          }
        }
  ...
  if PRINT_INDIVIDUAL_MESSAGES && task_messages_sent == num_messages_for_this_task {
    println!(
      "[DEALER {}] All {} sends initiated by this task.",
...
async fn main() -> Result<(), ZmqError> {
  ...
  router_socket
    ...
    .await?;
  router_socket
    .set_option(zmq_opts::IO_URING_RCVMULTISHOT, ROUTER_IO_URING_ENABLED)
    .await?;
  router_socket
    .set_option(
      zmq_opts::IO_URING_SNDZEROCOPY,
      SNDZEROCPY_IO_URING_ENABLED && ROUTER_IO_URING_ENABLED,
    )
    .await?;
  router_socket.set_option(zmq_opts::TCP_CORK, TCP_CORK_ENABLED).await?;
  router_socket
    .set_option(zmq_opts::RCVHWM, (TOTAL_MESSAGES_EXPECTED_BY_ROUTER as i32).max(5000))
    .await?;
  router_socket
    .set_option(zmq_opts::SNDHWM, (TOTAL_MESSAGES_EXPECTED_BY_ROUTER as i32).max(5000))
    .await?;
  router_socket
    .set_option(zmq_opts::RCVTIMEO, ROUTER_RECV_TIMEOUT_MS as i32)
    .await?;
  ...
  dealer_socket_main
    ...
    .await?;
  dealer_socket_main
    .set_option(zmq_opts::IO_URING_RCVMULTISHOT, DEALER_IO_URING_ENABLED)
    .await?;
  dealer_socket_main
    .set_option(
      zmq_opts::IO_URING_SNDZEROCOPY,
      SNDZEROCPY_IO_URING_ENABLED && DEALER_IO_URING_ENABLED,
    )
    .await?;
  dealer_socket_main
  ...
  dealer_socket_main
    .set_option(zmq_opts::RCVTIMEO, CENTRAL_RECEIVER_SOCKET_RECV_TIMEOUT_MS as i32)
    .await?;
  ...
  common::wait_for_handshake_events(
    vec![
      (
        ...
        String::from("ROUTER MAIN"),
      ),
      (
        dealer_monitor_rx,
        ROUTER_ENDPOINT.to_string(),
        String::from("DEALER MAIN"),
      ),
    ],
  ...
  println!(
    "[DEALER Main] All dealer tasks finished SENDING. Total successfully sent: {}",
    total_messages_successfully_sent
  );
  ...
  if total_messages_successfully_sent > 0 {
    loop {
      ...
      if num_pending_now == 0 {
        println!(
          "[Main] All {} successfully sent requests have been accounted for (0 pending in map).",
          total_messages_successfully_sent
        );
        break;
      }
      if receive_phase_monitor_start_time.elapsed() >= overall_receive_timeout_duration {
        println!(
          "[Main] Overall receive timeout ({}ms) reached. Pending requests: {}",
          overall_receive_timeout_duration.as_millis(),
          num_pending_now
        );
        break;
      }
      if central_receiver_handle.is_finished() {
        // Check if central receiver exited early
        println!(
          "[Main] Central receiver task finished while main was monitoring. Pending requests: {}",
          num_pending_now
        );
        break;
      }
  ...
  println!(
    "\n--- Overall Combined Results (Active Phase Duration: {:?}) ---",
    active_phase_duration
  );
  println!(
    "Benchmark Full Duration (incl. setup/teardown): {:?}",
    benchmark_active_phase_start_time.elapsed()
  );
  println!(
    "Successful DEALER tasks (for sending): {}/{}",
    successful_dealer_tasks, NUM_DEALER_TASKS
  );
  println!(
    "Total messages initiated by dealers: {} (send errors: {})",
    total_messages_sent_agg, total_send_errors_agg
  );
  println!(
    "Total messages successfully sent by dealers: {}",
    total_messages_successfully_sent
  );
  println!(
    "Total replies processed by central receiver: {}",
    central_replies_processed
  );
  println!(
    "Total reply extraction errors by central receiver: {}",
    central_extraction_errors
  );
  ...
  println!(
    "Implied lost/timed-out messages (successfully_sent - replied - pending_at_end): {}",
    lost_messages
  );
  ...
  if successful_dealer_tasks > 0 && active_seconds > 0.0 {
    ...
    println!(
      "Aggregate Send Rate (during active phase): {:.2} msgs/sec, {:.2} MB/sec",
    ...
    println!(
      "Effective System Recv Rate (during active phase): {:.2} msgs/sec, {:.2} MB/sec",
      agg_recv_msg_rate, agg_recv_byte_rate_mb
    );
  }
  if total_messages_successfully_sent != central_replies_processed
    || total_send_errors_agg > 0
    || central_extraction_errors > 0
    || final_num_pending > 0
  {
  ...
  match router_join_handle.await {
    Ok(Ok((router_msgs_echoed, router_bytes_echoed, router_timeouts))) => {
      println!(
        "[ROUTER] Task completed. Msgs echoed: {}. Bytes: {}. Timeouts: {}",
      ...
      if active_seconds > 0.0 {
        ...
        println!(
          "[ROUTER]: Effective echo rate (during active phase): {:.2} msgs/sec, {:.2} MB/sec",
          router_echo_msg_rate, router_echo_byte_rate_mb
        );
      }
      if router_msgs_echoed != total_messages_successfully_sent {
        eprintln!(
          "[ROUTER] WARNING: Router echoed {} msgs, dealers successfully sent {}.",
          router_msgs_echoed, total_messages_successfully_sent
        );
      }
      if router_msgs_echoed != central_replies_processed {
        eprintln!(
          "[ROUTER] WARNING: Router echoed {} msgs, central dealer received {} replies.",
          router_msgs_echoed, central_replies_processed
        );
      }
    }
