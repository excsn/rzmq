...
const SNDZEROCPY_IO_URING_ENABLED: bool = false; // Set to false to avoid fallback path logging if payload is small
...
const ROUTER_ENDPOINT: &'static str = "tcp://127.0.0.1:5559"; // New port
...
// --- Type Aliases ---
type RequestId = u64;
type PendingRequestsMap = Arc<TokioMutex<HashMap<RequestId, (OwnedSemaphorePermit, Instant)>>>;
...
async fn run_dealer_sender_task(
    ...
    first_send_ts: Arc<TokioMutex<Option<Instant>>>,
    ...
    for i in 0..num_messages {
        ...
        // Record timestamp of the very first send across all tasks
        let mut ts_guard = first_send_ts.lock().await;
        if ts_guard.is_none() {
            *ts_guard = Some(Instant::now());
        }
        drop(ts_guard);

        pending_map.lock().await.insert(request_id, (permit, Instant::now()));

        let frames = create_request_frames(request_id, &payload);
        dealer_socket.send_multipart(frames).await?;
    }
    Ok(DealerSenderStats { messages_sent: num_messages })
}

// --- NEW HELPER: Type alias for the complex future type in FuturesUnordered ---
type RecvFuture<'a> = Pin<Box<dyn Future<Output = (usize, Result<Vec<Msg>, ZmqError>)> + Send + 'a>>;
...
async fn run_dealer_central_receiver(
    ...
    pending_map: PendingRequestsMap,
    last_reply_ts: Arc<TokioMutex<Option<Instant>>>,
) -> u64 {
    ...
    while let Some((socket_idx, Ok(frames))) = recv_futures.next().await {
        ...
        if let Some((request_id, _)) = extract_reply_data(frames) {
            if let Some((permit, _)) = pending_map.lock().await.remove(&request_id) {
                drop(permit); // Release the semaphore permit
                replies_processed += 1;
                *last_reply_ts.lock().await = Some(Instant::now());
            }
        }
    }
    replies_processed
...
async fn main() -> Result<(), ZmqError> {
    ...
    router_socket.set_option(zmq_opts::SNDHWM, (TOTAL_MESSAGES) as i32).await?;
    router_socket.set_option(zmq_opts::RCVHWM, (TOTAL_MESSAGES) as i32).await?;
    let mut router_monitor = router_socket.monitor_default().await?;
    router_socket.bind(ROUTER_ENDPOINT).await?;
    let router_handle = tokio::spawn(run_router_task(router_socket));

    // --- Setup DEALERs ---
    let mut dealer_sockets = Vec::new();
    for i in 0..NUM_DEALERS {
        let dealer = ctx.socket(SocketType::Dealer)?;
        dealer.set_option(zmq_opts::ROUTING_ID, format!("dealer_{}", i).as_bytes()).await?;
        dealer.set_option(zmq_opts::IO_URING_SESSION_ENABLED, DEALER_IO_URING_ENABLED).await?;
        dealer.set_option(zmq_opts::TCP_CORK, TCP_CORK_ENABLED).await?;
        dealer.set_option(zmq_opts::SNDHWM, MAX_CONCURRENT_REQUESTS as i32).await?;
        dealer.set_option(zmq_opts::RCVHWM, MAX_CONCURRENT_REQUESTS as i32).await?;
    ...
    for _ in 0..NUM_DEALERS {
        common::wait_for_event(&mut router_monitor, Duration::from_millis(HANDSHAKE_TIMEOUT_MS), |e| matches!(e, SocketEvent::HandshakeSucceeded {..})).await.expect("Handshake timeout");
    }
    ...
    let first_send_ts = Arc::new(TokioMutex::new(None));
    let last_reply_ts = Arc::new(TokioMutex::new(None));
    ...
    let end_ts = last_reply_ts.lock().await.unwrap_or(start_ts);
    let total_duration = end_ts.duration_since(start_ts);
    ...
    if total_sent != receiver_count || total_sent != router_echoed {
        eprintln!("**WARNING: Message loss detected! Sent: {}, Received: {}, Router Echoed: {}**", total_sent, receiver_count, router_echoed);
    }
