...
fn create_request_frames(request_id: RequestId, payload_template: &Bytes) -> Vec<Msg> {
  ...
  let payload_msg = Msg::from_bytes(payload_template.clone());
...
async fn main() -> Result<(), ZmqError> {
  ...
  router_socket
    ...
    .await?;
  let mut router_monitor_rx = router_socket.monitor_default().await?;
  ...
  dealer_socket_main
    ...
    .await?;
  let mut dealer_monitor_rx = dealer_socket_main.monitor_default().await?;
